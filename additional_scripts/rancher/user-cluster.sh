...
yaml_path="/root/user-cluster.yml"
...
yaml=$(cat $yaml_path | sed -e 's=agent_image='"$agent_image"'=')
...
then
    # Uncomment the env var for dry-run mode
    echo "cattle-system namespace would be finally deleted"
    echo "Following CRDs with its resources would be deleted: "
    for crd in $(kubectl get crds | grep "cattle.io" | awk '{print $1}'); do echo ${crd}; done
else
    echo "Deleting cattle-system namespace"
    kubectl delete namespace cattle-system || echo "namespace cattle-system already gone"
    echo "Deleting rest of Rancher related CRDs"
    for crd in $(kubectl get crds | grep "cattle.io" | awk '{print $1}'); do kubectl delete crd ${crd}; done
fi
